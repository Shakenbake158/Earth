...
using Texture2D = Earth.Renderer.Texture2D;

namespace Earth.Scene
{
    internal class ClipmapUpdater : IDisposable
    {
        public ClipmapUpdater(ClipmapLevel[] clipmapLevels)
        {
            ...
            UnityEngine.Mesh unitQuad = RectangleTessellator.Compute(new RectangleD(new Vector2D(0.0, 0.0), new Vector2D(1.0, 1.0)), 1, 1);
            _unitQuad = unitQuad;
            MeshFilter updateMeshFilter = new MeshFilter();
            updateMeshFilter.mesh = _unitQuad;
            
            
            _unitQuadPrimitiveType = Earth.Core.PrimitiveType.Triangles;
            ...
            InitializeRequestThreads(_terrain, levelZero, levelZero.Terrain);
            InitializeRequestThreads(_imagery, levelZero, levelZero.Imagery);
        }

        private void InitializeRequestThreads(RasterDataDetails details, ClipmapLevel clipmapLevelZero, RasterLevel rasterLevelZero)
        {
        ...
        private UnityEngine.Mesh _unitQuad;
        ...
        private UnityEngine.Material _updateMaterial;
        private UnityEngine.Material _upsampleMaterial;
        private UnityEngine.Material _computeNormalsMaterial;
