...
using UnityEngine;
using PrimitiveType = Earth.Core.PrimitiveType;
using Random = System.Random;
using TextureFormat = Earth.Renderer.TextureFormat;

public class GlobeClipmapTerrain : IDisposable
{
    private Earth.Scene.RasterSource _terrainSource;
    private int _clipmapPosts;
    private int _clipmapSegments;
    private Earth.Scene.ClipmapLevel[] _clipmapLevels;

    public Material globeClipmapTerrain;
    ...
    private UnityEngine.Mesh _fillPatch;
    private UnityEngine.Mesh _horizontalFixupPatch;
    private UnityEngine.Mesh _verticalFixupPatch;
    private UnityEngine.Mesh _horizontalOffsetPatch;
    private UnityEngine.Mesh _verticalOffsetPatch;
    private UnityEngine.Mesh _centerPatch;
    private UnityEngine.Mesh _degenerateTrianglePatch;
    ...
    private Earth.Scene.ClipmapUpdater _updater;
    ...
    public GlobeClipmapTerrain(Earth.Scene.RasterSource terrainSource, Earth.Scene.EsriRestImagery imagery, Ellipsoid ellipsoid, int clipmapPosts)
    {
        ...
        _clipmapLevels = new Earth.Scene.ClipmapLevel[clipmapLevels];
        
        for (int i = 0; i < _clipmapLevels.Length; ++i)
        {
            _clipmapLevels[i] = new Earth.Scene.ClipmapLevel();
        }
        
        for (int i = 0; i < _clipmapLevels.Length; ++i)
        {
            Earth.Scene.RasterLevel terrainLevel = _terrainSource.Levels[i];
            ...
            Earth.Scene.RasterLevel imageryLevel = null;
        ...
        UnityEngine.Mesh fieldBlockMesh = RectangleTessellator.Compute(
        ...
        _fillPatch = fieldBlockMesh;

        // Create the Mx3 block used to fill the space between the MxM blocks in the ring
        UnityEngine.Mesh ringFixupHorizontalMesh = RectangleTessellator.Compute(
        ...
        _horizontalFixupPatch = ringFixupHorizontalMesh;
        
        // Create the 3xM block used to fill the space between the MxM blocks in the ring
        UnityEngine.Mesh ringFixupVerticalMesh = RectangleTessellator.Compute(
        ...
        _verticalFixupPatch = ringFixupVerticalMesh;
        
        UnityEngine.Mesh offsetStripHorizontalMesh = RectangleTessellator.Compute(
        ...
        _horizontalOffsetPatch = offsetStripHorizontalMesh;
        
        UnityEngine.Mesh offsetStripVerticalMesh = RectangleTessellator.Compute(
        ...
        _verticalOffsetPatch = offsetStripVerticalMesh;
        
        UnityEngine.Mesh centerMesh = RectangleTessellator.Compute(new RectangleD(new Vector2D(0.0, 0.0), new Vector2D(2.0, 2.0)), 2, 2);
        //_centerPatch = context.CreateVertexArray(centerMesh, _shaderProgram.VertexAttributes, BufferHint.StaticDraw);
        _centerPatch = centerMesh;
        
        UnityEngine.Mesh degenerateTriangleMesh = CreateDegenerateTriangleMesh();
        ...
        _degenerateTrianglePatch = degenerateTriangleMesh;
        ...
        _renderState.FacetCulling.FrontFaceWindingOrder = Earth.Core.WindingOrder.Counterclockwise;
        _primitiveType = Earth.Core.PrimitiveType.Triangles;
        ...
        _updater = new Earth.Scene.ClipmapUpdater(_clipmapLevels);
    ...
    public void PreRender(/*Context context, SceneState sceneState*/)
    {
        ...
        Earth.Scene.ClipmapLevel level = _clipmapLevels[_clipmapLevels.Length - 1];
        ...
        for (int i = _clipmapLevels.Length - 2; i >= 0; --i)
        {
            ...
            Earth.Scene.ClipmapLevel finerLevel = _clipmapLevels[i + 1];
        ...
        for (int i = 0; i <_clipmapLevels.Length; ++i)
        {
            Earth.Scene.ClipmapLevel thisLevel = _clipmapLevels[i];
            Earth.Scene.ClipmapLevel coarserLevel = _clipmapLevels[i > 0 ? i - 1 : 0];
    ...
    private void UpdateOriginInTextures(Earth.Scene.ClipmapLevel level)
    {
    ...
    private void UpdateImageryOriginInTextures(Earth.Scene.ClipmapLevel level)
    {
    ...
    private void UpdateTerrain(Earth.Scene.ClipmapLevel level, Earth.Scene.ClipmapLevel coarserLevel, Context context, SceneState sceneState)
    {
        int deltaX = level.NextExtent.West - level.CurrentExtent.West;
        int deltaY = level.NextExtent.South - level.CurrentExtent.South;
        if (deltaX == 0 && deltaY == 0)
        ...
        if (height > 0)
        {
            Earth.Scene.ClipmapUpdate horizontalUpdate = new Earth.Scene.ClipmapUpdate(
                level,
                level.NextExtent.West,
                minLatitude,
        ...
        if (width > 0)
        {
            Earth.Scene.ClipmapUpdate verticalUpdate = new Earth.Scene.ClipmapUpdate(
                level,
                minLongitude,
                level.NextExtent.South,
                maxLongitude,
    ...
    private void UpdateImagery(Earth.Scene.ClipmapLevel level, Earth.Scene.ClipmapLevel coarserLevel, Context context, SceneState sceneState)
    {
        int deltaX = level.NextImageryExtent.West - level.CurrentImageryExtent.West;
        int deltaY = level.NextImageryExtent.South - level.CurrentImageryExtent.South;
        if (deltaX == 0 && deltaY == 0)
    ...
    public void Render(/* context, SceneState sceneState*/)
    {
        if (_wireframe)
        {
    ...
    private bool RenderLevel(int levelIndex, Earth.Scene.ClipmapLevel level, Earth.Scene.ClipmapLevel coarserLevel, bool fillRing, Vector2D center, Context context, SceneState sceneState)
    {
        /*
    ...
    private void DrawBlock(VertexArray block, Earth.Scene.ClipmapLevel level, Earth.Scene.ClipmapLevel coarserLevel, int overallWest, int overallSouth, int blockWest, int blockSouth, Context context, SceneState sceneState)
    {
    ...
    private UnityEngine.Mesh CreateDegenerateTriangleMesh()
    {
        ...
        {
            positions.Add(new UnityEngine.Vector2(0.0f, i));
        }

        for (int i = 1; i < _clipmapPosts; ++i)
        {
            positions.Add(new UnityEngine.Vector2(i, _clipmapSegments));
        }

        for (int i = _clipmapSegments - 1; i >= 0; --i)
        {
            positions.Add(new UnityEngine.Vector2(_clipmapSegments, i));
        }

        for (int i = _clipmapSegments - 1; i > 0; --i)
        {
            positions.Add(new UnityEngine.Vector2(i, 0.0f));
        }
    ...
    public void Dispose()
    {
        foreach (Earth.Scene.ClipmapLevel level in _clipmapLevels)
        {
            level.Dispose();
        }
